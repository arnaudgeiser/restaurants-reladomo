...
public class CLI {
  ...
  private Optional<Restaurant> pickRestaurant(Set<Restaurant> restaurants) {
    ...
    String restaurantsText = restaurants.stream()
        .map(r -> "\"" + r.getName() + "\" - " + r.getStreet() + " - "
            + r.getCity().getZipCode() + " " + r.getCity().getCityName())
        .collect(joining("\n", "", "\n"));
  ...
  private void showRestaurantsList() {
    ...
    if (maybeRestaurant.isPresent()) { // Si l'utilisateur a choisi un restaurant, on l'affiche, sinon on ne fait rien et l'application va réafficher le menu principal
      Restaurant restaurantToDisplay = restaurantService.researchById(maybeRestaurant.get().getId());
  ...
  private void addNewRestaurant() {
    ...
    restaurant.setName(description);
    restaurant.setName(website);
  ...
  private void showRestaurant(Restaurant restaurant) {
    String sb = restaurant.getName() + "\n" +
        ...
        restaurant.getCity().getZipCode() + " " + restaurant.getCity()
        .getCityName() + "\n" +
  ...
  private void addBasicEvaluation(Restaurant restaurant, Boolean like) {
    ...
    try {
      ipAddress = Inet4Address.getLocalHost()
          .toString(); // Permet de retrouver l'adresse IP locale de l'utilisateur.
    } catch (UnknownHostException ex) {
      Logger.getLogger(CLI.class.getName())
          .log(Level.INFO, "Error - Couldn't retreive host IP address");
    ...
    restaurant.getBasicEvaluations().add(eval);
  ...
  private void evaluateRestaurant(Restaurant restaurant) {
    ...
    Set<EvaluationCriteria> evaluationCriterias = restaurantService
        .researchAllEvaluationCritierias();
    ...
    evaluationCriterias.forEach(currentCriteria -> {
      ...
      grade.setCriteria(currentCriteria);
  ...
  private void editRestaurantAddress(Restaurant restaurant) {
    ...
    restaurant.setStreet(readString());

    Set<City> cities = restaurantService.researchAllCities();

    City newCity = pickCity(cities);
    if (newCity.equals(restaurant.getCity())) {
      restaurant.getCity().removeRestaurant(restaurant);
